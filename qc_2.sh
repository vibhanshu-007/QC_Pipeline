...
with open(depth_file) as f:
    ...
    for row in reader:
        ...
        rows.append({
            ...
            "Pre_QC_Status_MD5_Readcount_Actual_Depth": "; ".join(remarks),
...
run_quality_metrics() {
    ...
    for sample in "${!READCOUNT_STATUS[@]}"; do
        ...
        pct20=$(awk "BEGIN {printf \"%.2f\", 100 * $q20 / $total}")
        pct30=$(awk "BEGIN {printf \"%.2f\", 100 * $q30 / $total}")
        pct40=$(awk "BEGIN {printf \"%.2f\", 100 * $q40 / $total}")
